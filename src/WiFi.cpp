...
void WiFiClass::printDiag(Print& p)
{
    ...
    const char*      modes[] = { "NULL", "STA", "AP", "STA+AP" };
    extern WifiToken token;
    int32_t          mode;
    WifiApConfigure  local;

    atWifiMode(& mode);
    atWifiApConfigure(& local);

    p.print("Mode: ");
    p.println(modes[mode]);

    p.print("Channel: ");
    p.println(local.channel);
    ...
    */
    ...
    p.print("BSSID set: ");
    p.printf("%02X:%02X:%02X:%02X:%02X:%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
