...
static wl_status_t wifiBegin(bool _useStaticIp, bool connect){
    bool result;
    ...
    if (!_useStaticIp) {
        ...
        atDhcp(disable, 1 << TCPIP_ADAPTER_IF_STA);
    }

    if (connect == false) {
        return WiFiSTAClass::status();
    }
...
wl_status_t WiFiSTAClass::begin(){
    ...
    return wifiBegin(_useStaticIp, status() != WL_CONNECTED);
}
 ...
 */
uint8_t* WiFiSTAClass::macAddress(uint8_t* bssid){
    // if(WiFiGenericClass::getMode() != WIFI_MODE_NULL){
