...
void WiFiScanClass::_scanDone() {
    ...
    if (WiFiScanClass::_scanCount) {
        ...
        if (!WiFiScanClass::_scanResult
                || esp_wifi_scan_get_ap_records(&(WiFiScanClass::_scanCount), (wifi_ap_record_t*)_scanResult) != ESP_OK) {
...
int16_t WiFiScanClass::scanComplete() {
    if (WiFiScanClass::_scanStarted
            && (millis() - WiFiScanClass::_scanStarted) > WiFiScanClass::_scanTimeout) { //Check is scan was started and if the delay expired, return WIFI_SCAN_FAILED in this case
        WiFiGenericClass::clearStatusBits(WIFI_SCANNING_BIT);
        return WIFI_SCAN_FAILED;
    }
...
/**
    ...
    @return (true if ok)
*/
bool WiFiScanClass::getNetworkInfo(uint8_t i, String& ssid, uint8_t& encType, int32_t& rssi, uint8_t*& bssid,
                                   int32_t& channel) {
...
String WiFiScanClass::BSSIDstr(uint8_t i) {
    ...
    sprintf(mac, "%02X:%02X:%02X:%02X:%02X:%02X", it->bssid[0], it->bssid[1], it->bssid[2], it->bssid[3], it->bssid[4],
            it->bssid[5]);
    return String(mac);
}
